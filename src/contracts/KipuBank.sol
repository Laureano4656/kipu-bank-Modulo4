...
contract KipuBank is AccessControl, ReentrancyGuard {
    ...
    error InsufficientBalance(address token, address user, uint256 available, uint256 required);
    error WithdrawExceedsPerTx(address token, uint256 limitUsd6, uint256 attemptedUsd6);
    ...
    /// Events
    event Deposit(address indexed token, address indexed user, uint256 amount, uint256 usdValue6);
    event Withdrawal(address indexed token, address indexed user, uint256 amount, uint256 usdValue6);
    ...
    constructor(address admin, address ethUsdPriceFeed, uint256 bankCapUsd6, uint256 maxWithdrawUsd6) {
        ...
        i_uniswapRouter = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // Uniswap V2 Router
        // mainnet address: 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f
        // sepoila address: 0xF62c03E08ada871A0bEb309762E260a7a6a880E6
        i_uniswapFactory = IUniswapV2Factory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f); // Uniswap V2 Factory.
        s_totalUsdStored6 = 0;
        s_totalDeposits = 0;
    ...
    function setMaxWithdrawUsd6(uint256 newMaxUsd6) external onlyRole(ADMIN_ROLE) {
    ...
    function _getPathForTokenToUsd6(address token) internal view returns (address[] memory) {
        address[] memory path;

        address pairAddress = i_uniswapFactory.getPair(token, USDC_ADDRESS);
        if (pairAddress == address(0)) {
            pairAddress = i_uniswapFactory.getPair(token, i_uniswapRouter.WETH());
    ...
    function depositToken(address token, uint256 amount, uint256 minOut) external nonReentrant {
        if (amount == 0) revert ZeroAmount();
        ...
        // Estimate output
        uint256[] memory amountsOut = i_uniswapRouter.getAmountsOut(amount, path);
        ...
        IERC20Metadata(token).safeIncreaseAllowance(address(i_uniswapRouter), amount);

        // Swap to USDC
        uint256[] memory amounts =
            i_uniswapRouter.swapExactTokensForTokens(amount, minOut, path, address(this), block.timestamp);
    ...
    function withdrawETH(uint256 amount) external nonReentrant {
        ...
        if (userBal < amount) {
            revert InsufficientBalance(NATIVE_TOKEN, msg.sender, userBal, amount);
        }
    ...
    function withdrawUSDC(uint256 amount) external nonReentrant {
        ...
        if (userBal < amount) {
            revert InsufficientBalance(USDC_ADDRESS, msg.sender, userBal, amount);
        }
    ...
    function _calculateCurrentTVL() internal view returns (uint256) {
        ...
        uint256 usdcBalance = IERC20Metadata(USDC_ADDRESS).balanceOf(address(this));
    ...
    function _ethToUsd6(uint256 amount) internal view returns (uint256) {
        ...
        uint256 denom = (10 ** uint256(tokenDecimals)) * (10 ** uint256(priceDecimals));
